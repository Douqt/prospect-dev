...
export default function MentorDashboardClient() {
  ...
  const fetchMentorData = async (mentorId: string) => {
    try {
      setLoading(true);
      await Promise.all([
        fetchCourses(mentorId),
        fetchAnalytics(mentorId),
      ]);
    } catch (error) {
  ...
  const fetchCourses = async (mentorId: string) => {
    try {
  ...
  const fetchAnalytics = async (mentorId: string) => {
    try {
  ...
  const handleDeleteCourse = async (courseId: string) => {
    if (!confirm('Are you sure you want to delete this course? This action cannot be undone and will remove all lessons and student enrollments.')) {
      return;
    }
  ...
  // Show access denied if not a mentor
  if (!user || user.role !== 'mentor') {
    return (
      <div className="text-center py-12">
        <h2 className="text-xl font-bold mb-2">Access Denied</h2>
        <p className="text-gray-600">
          You need mentor privileges to access this dashboard.
        </p>
      </div>
    );
  }
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="trailer">Trailer Video ID (Vimeo)</Label>
                  <Input
                    id="trailer"
                    value={courseForm.trailer_video_id || ''}
                    onChange={(e) => setCourseForm({ ...courseForm, trailer_video_id: e.target.value })}
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="edit-trailer">Trailer Video ID (Vimeo)</Label>
                  <Input
                    id="edit-trailer"
                    value={courseForm.trailer_video_id || ''}
                    onChange={(e) => setCourseForm({ ...courseForm, trailer_video_id: e.target.value })}
        ...
        <TabsContent value="lessons" className="space-y-6">
          {!managingLessonsCourse ? (
            courses.length === 0 ? (
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="outline" onClick={() => setManagingLessonsCourse(null)}>
                    Back to Courses
                  </Button>
                </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-medium">Managing Lessons for: {managingLessonsCourse.title}</h3>
                  <p className="text-sm text-gray-600">{managingLessonsCourse.description}</p>
                </div>
                <Button variant="outline" onClick={() => setManagingLessonsCourse(null)}>
                  Back to Courses
                </Button>
              ...
              <LessonManager courseId={managingLessonsCourse.id} mentorId={user.id} />
            </div>
          )}
        </TabsContent>

        <TabsContent value="earnings" className="space-y-6">
          <EarningsTracker mentorId={user.id} />
        </TabsContent>

        <TabsContent value="settings" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <StripeConnectOnboarding mentorId={user.id} />
          </div>
        </TabsContent>
      </Tabs>
    </div>

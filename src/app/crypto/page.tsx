...
import { CRYPTO_FORUMS, isCryptoForum, isFuturesForum } from "../../../forum-categories";
...
export default function CryptoPage() {
  ...
  useEffect(() => {
    const preloadCharts = async () => {
      try {
        console.log('🔥 Preloading charts for crypto feed symbols...');
  ...
  const { data: discussions, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (feedType === 'following' && user) {
        ...
        // Additional filtering: only show followed forums that are actually crypto forums
        const cryptoFollowingDiscussions = data.filter(d => {
          const category = d.category?.toUpperCase() || '';
          const isCryptoForum = CRYPTO_FORUMS.includes(category);
          console.log(`Following category ${category}: is crypto forum? ${isCryptoForum}`);
          return isCryptoForum;
        });

        console.log('Filtered following crypto discussions:', cryptoFollowingDiscussions.length, 'out of', data.length);

        const discussionsWithProfiles = await Promise.all(
          cryptoFollowingDiscussions.map(async (discussion) => {
        ...
        // Filter only crypto forums
        console.log('Available categories in data:', data.map(d => d.category));
        const cryptoDiscussions = data.filter(d => {
          const category = d.category?.toUpperCase() || '';
          const isCryptoForum = CRYPTO_FORUMS.includes(category);
          console.log(`Category ${category}: is crypto forum? ${isCryptoForum}`);
          return isCryptoForum;
        });

        console.log('Filtered crypto discussions:', cryptoDiscussions.length, 'out of', data.length);

        const discussionsWithProfiles = await Promise.all(
          cryptoDiscussions.map(async (discussion) => {
            const { data: profile } = await supabase
              .from("profiles")
              .select("username, display_name, avatar_url")
              .eq("id", discussion.user_id)
          ...
          <div className="border border-[#e0a815]/50 rounded-lg overflow-hidden">
            {isLoading ? (
              <Card className="text-center py-12 border-0 rounded-none">
                <CardContent>
                  <div className="mb-4">
                    <div className="text-muted-foreground">Loading crypto feed...</div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div className="mb-4">
                    ...
                    <h3 className="text-xl font-semibold mb-2">
                      {feedType === 'following' ? "No posts from followed forums yet" : "No crypto posts yet"}
                    </h3>
                    <p className="text-muted-foreground mb-4">
                      {feedType === 'following'
                        ? "Start following crypto traders to see their posts here"
                        : "Be the first to post in our crypto trading community"
                      }
                    </p>
                  </div>
                </CardContent>
                          ...
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                              ...
                              <span
                                onClick={(e) => {
                                  e.stopPropagation();
                                  window.location.href = `/crypto/${discussion.category.toLowerCase()}`;
                                }}

  ...
  const { data: discussion, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("discussions")
        .select("id, title, content, category, created_at, upvotes, downvotes, comment_count, user_id, main_category, image_url")
        ...
        <main className="relative z-10 pt-24 pl-64 pr-6">
          <div className="max-w-4xl mx-auto">
            <div className="text-center py-16">
              ...
              <Link href={`/futures/${resolvedParams?.symbol || 'general'}`}>
                <Button>
    ...
    <div className="min-h-screen bg-background relative overflow-hidden text-foreground">
      ...
      <main className="relative z-10 pt-24 pl-64 pr-6">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {/* Back Button */}
          <Link href={`/futures/${resolvedParams?.symbol || 'general'}`} className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
            <ArrowLeft className="w-4 h-4" />
            Back to {resolvedParams?.symbol?.toUpperCase() || 'Futures'} Forum
          </Link>
          ...
          <Card>
            <CardContent className="p-6">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <Badge variant="outline" className="text-xs">
                      <Link href={`/futures/${discussion.category.toLowerCase()}`}>
              ...
              {/* Post Image (stored in image_url field) */}
              {discussion.image_url && (
                <div className="mb-6 -mx-6">
                  <img
                    ...
                    onLoad={() => console.log('✅ Post image loaded successfully:', discussion.image_url)}
                    onError={(e) => {
                      console.log('❌ Post image failed to load:', discussion.image_url);
